...
export function useTrackFocusPath(props: Props): void {
  ...
  useEffect(() => {
    ...
    if (openItem && openItem.elementRef?.current) {
      ...
      // Place the cursor and focus the editor for focusPaths that terminate on editor elements
      if (isTextBlock || isBlockFocusPath) {
        // Find a text block child from the focusPath
        const childKey = isTextBlock && isKeyedObject(focusPath[2]) ? focusPath[2]._key : undefined
        const child =
          childKey && Array.isArray(openItem.node.value?.children)
            ? (openItem.node.value?.children.find((c) => c._key === childKey) as
                | KeyedObject
                | undefined)
            : undefined

        // Is focus pointing to span's `.text` property? That should have focus as well.
